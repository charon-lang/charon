...
typedef struct {
    const query_descriptor_t *descriptor;
    size_t bucket_count;
    query_state_t **buckets;
} query_descriptor_state_t;

struct query_engine {
    charon_memory_allocator_t *allocator;
    context_t *context;

    query_descriptor_state_t *descriptors;
...
static uint64_t hash_key(const query_descriptor_t *descriptor, const void *key) {
...
static bool keys_equal(const query_descriptor_t *descriptor, const void *lhs, const void *rhs) {
...
static void pointer_array_push(query_engine_t *engine, query_state_t ***array, size_t *count, size_t *capacity, query_state_t *value) {
    if(*count == *capacity) {
        size_t new_capacity = *capacity == 0 ? 4 : (*capacity * 2);
        *array = charon_memory_allocate_array(engine->allocator, *array, new_capacity, sizeof(query_state_t *));
        *capacity = new_capacity;
    }
    (*array)[(*count)++] = value;
}

static void pointer_array_remove(query_state_t ***array, size_t *count, query_state_t *value) {
    if(*count == 0) return;
    for(size_t i = 0; i < *count; ++i) {
        if((*array)[i] != value) continue;
        memmove(&(*array)[i], &(*array)[i + 1], ((*count) - i - 1) * sizeof(query_state_t *));
        --(*count);
        return;
    }
}

static void engine_stack_push(query_engine_t *engine, query_state_t *state) {
    if(engine->stack_size == engine->stack_capacity) {
        size_t new_capacity = engine->stack_capacity == 0 ? 8 : engine->stack_capacity * 2;
        engine->stack = charon_memory_allocate_array(engine->allocator, engine->stack, new_capacity, sizeof(query_state_t *));
        engine->stack_capacity = new_capacity;
    }
    engine->stack[engine->stack_size++] = state;
}

static void engine_stack_pop(query_engine_t *engine) {
    assert(engine->stack_size != 0);
    engine->stack_size--;
}

static query_descriptor_state_t *engine_get_descriptor_state(query_engine_t *engine, const query_descriptor_t *descriptor, bool create) {
    ...
    if(engine->descriptor_count == engine->descriptor_capacity) {
        ...
        engine->descriptors = charon_memory_allocate_array(engine->allocator, engine->descriptors, new_capacity, sizeof(query_descriptor_state_t));
        engine->descriptor_capacity = new_capacity;
    }

    query_descriptor_state_t *state = &engine->descriptors[engine->descriptor_count++];
    state->descriptor = descriptor;
    state->bucket_count = QUERY_DEFAULT_BUCKET_COUNT;
    state->buckets = charon_memory_allocate_array(engine->allocator, nullptr, state->bucket_count, sizeof(query_state_t *));
    for(size_t i = 0; i < state->bucket_count; ++i) state->buckets[i] = nullptr;

    return state;
}

static query_state_t *descriptor_state_find(query_descriptor_state_t *descriptor_state, const void *key, uint64_t hash) {
    size_t bucket = descriptor_state->bucket_count == 0 ? 0 : (hash % descriptor_state->bucket_count);
    query_state_t *state = descriptor_state->buckets[bucket];
    while(state != nullptr) {
...
static query_state_t *descriptor_state_insert(query_engine_t *engine, query_descriptor_state_t *descriptor_state, const void *key, uint64_t hash) {
    const query_descriptor_t *descriptor = descriptor_state->descriptor;
...
static void query_state_remove_from_dependencies(query_state_t *state) {
    for(size_t i = 0; i < state->dependency_count; ++i) {
        query_state_t *dependency = state->dependencies[i];
        pointer_array_remove(&dependency->dependents, &dependency->dependent_count, state);
    }
    state->dependency_count = 0;
}

static void query_state_remove_from_dependents(query_state_t *state) {
    for(size_t i = 0; i < state->dependent_count; ++i) {
        query_state_t *dependent = state->dependents[i];
        pointer_array_remove(&dependent->dependencies, &dependent->dependency_count, state);
    }
    state->dependent_count = 0;
}

static void query_state_add_dependency(query_engine_t *engine, query_state_t *parent, query_state_t *dependency) {
...
query_engine_t *query_engine_make(charon_memory_allocator_t *allocator, context_t *context) {
    ...
    query_engine_t *engine = charon_memory_allocate(allocator, sizeof(query_engine_t));
...
void query_engine_destroy(query_engine_t *engine) {
    ...
    for(size_t i = 0; i < engine->descriptor_count; ++i) {
        query_descriptor_state_t *descriptor_state = &engine->descriptors[i];
        for(size_t bucket = 0; bucket < descriptor_state->bucket_count; ++bucket) {
            query_state_t *state = descriptor_state->buckets[bucket];
            while(state != nullptr) {
                query_state_t *next = state->next;
...
bool query_engine_execute(query_engine_t *engine, const query_descriptor_t *descriptor, const void *key, const void **value_out) {
    ...
    query_descriptor_state_t *descriptor_state = engine_get_descriptor_state(engine, descriptor, true);
