...
static void resize_interner(interner_t *interner, size_t new_capacity) {
    ...
    for(size_t i = 0; i < interner->capacity; i++) {
        ...
        entry_t *new_entry =
            find_entry(entries, new_capacity, interner->entries[i].value, interner_value_size(interner, interner->entries[i].value), interner->entries[i].value_hash, interner->equality_fn);
...
interner_t *interner_new(size_t initial_capacity, size_t value_size, interner_size_fn_t size_fn, interner_hash_fn_t hash_fn, interner_equality_fn_t equality_fn) {
