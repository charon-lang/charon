...
static hlir_node_t *parse_type(tokenizer_t *tokenizer) {
    ...
    switch(token.kind) {
        case TOKEN_KIND_KEYWORD_STRUCT: {
            util_consume(tokenizer, TOKEN_KIND_KEYWORD_STRUCT);
        ...
        default: diag_error(util_loc(tokenizer, token), "invalid type");
    }
    __builtin_unreachable();
}

static hlir_node_t *parse_module(tokenizer_t *tokenizer) {
    ...
    return hlir_node_make_tlc_module(util_text_make_from_token(tokenizer, token_identifier), tlcs, source_location);
}

static hlir_node_t *parse_function(tokenizer_t *tokenizer) {
