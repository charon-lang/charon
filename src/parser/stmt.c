...
static hlir_node_t *parse_expression(tokenizer_t *tokenizer) {
    hlir_node_t *expression = parser_expr(tokenizer);
    return hlir_node_make_stmt_expression(expression, expression->source_location);
}
...
static hlir_node_t *parse_declaration(tokenizer_t *tokenizer) {
    ...
    return hlir_node_make_stmt_declaration(util_text_make_from_token(tokenizer, token_name), type, initial, source_location);
}

static hlir_node_t *parse_return(tokenizer_t *tokenizer) {
    ...
    return hlir_node_make_stmt_return(value, util_loc(tokenizer, token_return));
}

static hlir_node_t *parse_if(tokenizer_t *tokenizer) {
    ...
    return hlir_node_make_stmt_if(condition, body, else_body, util_loc(tokenizer, token_if));
}

static hlir_node_t *parse_while(tokenizer_t *tokenizer) {
    ...
    hlir_node_t *body = parser_stmt(tokenizer);
    return hlir_node_make_stmt_while(condition, body, util_loc(tokenizer, token_while));
}

static hlir_node_t *parse_block(tokenizer_t *tokenizer) {
    ...
    return hlir_node_make_stmt_block(statements, source_location);
}

static hlir_node_t *parse_simple(tokenizer_t *tokenizer) {
    hlir_node_t *node;
