...
int ir_type_cmp(ir_type_t *a, ir_type_t *b) {
    ...
    switch(a->kind) {
        ...
        case TYPE_KIND_INTEGER: return a->integer.bit_size - b->integer.bit_size;
        case TYPE_KIND_POINTER: return ir_type_cmp(a->pointer.base, b->pointer.base);
    }
    assert(false);
}
...
ir_type_t *ir_type_get_u8() {
    if(g_u8 == NULL) g_u8 = make_type(TYPE_KIND_INTEGER);
...
ir_type_t *ir_type_get_u16() {
    if(g_u16 == NULL) g_u16 = make_type(TYPE_KIND_INTEGER);
...
ir_type_t *ir_type_get_u32() {
    if(g_u32 == NULL) g_u32 = make_type(TYPE_KIND_INTEGER);
...
ir_type_t *ir_type_get_u64() {
    if(g_u64 == NULL) g_u64 = make_type(TYPE_KIND_INTEGER);
